...
$autori_view = NULL; // Initialize page object first

class cautori_view extends cautori {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "autorilist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "autorilist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "autorilist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($autori_view)) $autori_view = new cautori_view();

// Page init
$autori_view->Page_Init();

// Page main
$autori_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$autori_view->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var autori_view = new ew_Page("autori_view");
autori_view.PageID = "view"; // Page ID
var EW_PAGE_ID = autori_view.PageID; // For backward compatibility

// Form object
var fautoriview = new ew_Form("fautoriview");

// Form_CustomValidate event
fautoriview.Form_CustomValidate = 
...
fautoriview.ValidateRequired = true;
<?php } else { ?>
fautoriview.ValidateRequired = false; 
<?php } ?>
...
<?php $autori_view->ExportOptions->Render("body") ?>
<?php if (!$autori_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($autori_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php $autori_view->ShowPageHeader(); ?>
<?php
$autori_view->ShowMessage();
?>
<form name="fautoriview" id="fautoriview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="autori">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_autoriview" class="table table-bordered table-striped">
...
<?php echo $autori->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($autori->autor_eng->Visible) { // autor_eng ?>
	<tr id="r_autor_eng">
		<td><span id="elh_autori_autor_eng"><?php echo $autori->autor_eng->FldCaption() ?></span></td>
...
<?php echo $autori->autor_eng->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($autori->autor_mne->Visible) { // autor_mne ?>
	<tr id="r_autor_mne">
		<td><span id="elh_autori_autor_mne"><?php echo $autori->autor_mne->FldCaption() ?></span></td>
...
<?php echo $autori->autor_mne->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $autori->brojprezimena->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
